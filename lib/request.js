...
exports.initialize = function (settings, self) {
	...
	*/
	self.getRequestOptions = function (options) {
		var returnOptions = {};
		if(options[self.failover.key] ){
			var portIndex = options[self.failover.key].indexOf(':');
			if (portIndex > 0) {
				options.port = self.settings[self.failover.key].substr(portIndex + 1);
				options[self.failover.key] = self.settings[self.failover.key].substr(0, portIndex);
				self.settings.port = options.port;
				self.settings[self.failover.key] = options[self.failover.key]
			}
		}
